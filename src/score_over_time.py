...
def get_sorted_plays(df):
    ...
    df["game_seconds_elapsed"] = (df["quarter"] - 1) * 900 + (900 - df["seconds_remaining"])
    # Quarter ASC, Seconds DESC (clock counts down)
    return df.sort_values(by=["quarter", "seconds_remaining"], ascending=[True, False])

def plot_scores(df, target_game_id, home_team_name, visitor_team_name, final_home_score, final_visitor_score, output_path=None):
    ...
    df["home_expected"] = df.apply(
        lambda row: row["preSnapHomeScore"] + (row["expectedPoints"] if row["possessionTeam"] == home_team_name else 0),
        axis=1
    )
    df["visitor_expected"] = df.apply(
        lambda row: row["preSnapVisitorScore"] + (row["expectedPoints"] if row["possessionTeam"] == visitor_team_name else 0),
        axis=1
    )
    ...
    # --- Top Subplot: Scores ---
    # Plot Actual Scores
    ax1.step(df["game_seconds_elapsed"], df["preSnapHomeScore"], label=f"{home_team_name} Actual", where='post', color='blue', alpha=0.3)
    ax1.step(df["game_seconds_elapsed"], df["preSnapVisitorScore"], label=f"{visitor_team_name} Actual", where='post', color='red', alpha=0.3)

    # Plot Expected Scores
    ax1.plot(df["game_seconds_elapsed"], df["home_expected"], label=f"{home_team_name} Expected", color='blue', linewidth=2)
    ax1.plot(df["game_seconds_elapsed"], df["visitor_expected"], label=f"{visitor_team_name} Expected", color='red', linewidth=2)

    ax1.set_title(f"Score and Expected Points: {visitor_team_name} at {home_team_name} ({target_game_id})")
    ax1.set_ylabel("Points")
    ax1.legend()
    ax1.grid(True, linestyle='--', alpha=0.7)

    # --- Bottom Subplot: Net Difference (Winner is Positive) ---
    ax2.step(df["game_seconds_elapsed"], df["net_actual"], label="Actual Diff", where='post', color='black', alpha=0.2)
    ax2.plot(df["game_seconds_elapsed"], df["net_expected"], label="Expected Diff", color='purple', linewidth=1.5)
    
    # Shade regions to show who is "leading"
    ax2.fill_between(df["game_seconds_elapsed"], 0, df["net_expected"], 
                     where=(df["net_expected"] >= 0), color=pos_color, alpha=0.2, label=f"{pos_team} Lead", interpolate=True)
    ax2.fill_between(df["game_seconds_elapsed"], 0, df["net_expected"], 
                     where=(df["net_expected"] < 0), color=neg_color, alpha=0.2, label=f"{neg_team} Lead", interpolate=True)
    ...
    for ax in [ax1, ax2]:
        for q in range(1, 5):
            ...
            if ax == ax1:
                ax.text(q*900 - 450, ax.get_ylim()[1] * 0.95, f"Q{q}", horizontalalignment='center')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Plot scores and expected points over time for a specific game.")
    parser.add_argument("game_id", type=int, help="The ID of the game to plot.")
    parser.add_argument("-o", "--output", help="Path to save the output graph file (e.g., 'output.png'). If not provided, the graph will be displayed.")
    ...
    # Generate Plot
    plot_scores(df_sorted, game_id, home_team, visitor_team, home_final, visitor_final, output_path=output_path)
