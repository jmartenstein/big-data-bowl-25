...
for g in game_ids:
    ...
    for p in df_pass_plays_per_game:
        ...
        frame_id = ap.get_frame_id_for_event(df_play_frames, "pass_forward")
        if (frame_id < 0):
            frame_id = ap.get_frame_id_for_event(df_play_frames, "pass_shovel")

        passer_id = ap.find_player_id_by_closest_to_football(df_play_frames, frame_id)
